...
/// ======================================================================
/// Soccial Token – Integration Tests: Staking Vault Deposits
///
/// These integration tests validate the logic for securely depositing
/// tokens into the Staking Vault. The tests ensure that balances are
...
#[tokio::test]
async fn test_deposit_staking_vault_should_succeed() -> Result<(), TransportError> {
    ...
    try_deposit_staking_vault(&mut context, &admin, &participant, 1_000).await?;
...
#[tokio::test]
async fn test_deposit_staking_balances_should_succeed() -> Result<(), TransportError> {
    let (mut context, admin) = setup_test_env().await;
    ...
    let staking_before = context.get_vault_balance("staking").await;
    let participant_before = context.get_user_balance(&participant.pubkey()).await;

    // Execute the deposit
    try_deposit_staking_vault(&mut context, &admin, &participant, deposit_amount).await?;

    context.refresh().await;

    // Balances after the deposit
    let staking_after = context.get_vault_balance("staking").await;
    ...
    assert_eq!(
        ...
        deposit_amount,
        "❌ Staking vault should increase by {} tokens, but increased by {}",
        deposit_amount,
        staking_after - staking_before
    );
    ...
    msg!("✅ Staking vault deposit successful and balances verified.");
    log_all_balances(&mut context, &admin).await?;
...
#[tokio::test]
async fn test_deposit_staking_vault_without_tokens_should_fail() -> Result<(), TransportError> {
    ...
    let result = try_deposit_staking_vault(&mut context, &admin, &participant, 1_000).await;

    assert!(
        result.is_err(),
        "🚨 Expected failure when depositing staking without tokens."
    );
...
#[tokio::test]
async fn test_deposit_staking_vault_zero_tokens_should_fail() -> Result<(), TransportError> {
    ...
    let result = try_deposit_staking_vault(&mut context, &admin, &participant, 0).await;
...
#[tokio::test]
async fn test_deposit_staking_vault_unauthorized_should_fail() -> Result<(), TransportError> {
    ...
    let result = try_deposit_staking_vault(&mut context, &intruder, &intruder, 1_000).await;
    
    assert_custom_error(
        result,
        ErrorCode::Unauthorized,
        "Expected failure when unauthorized user tried to deposit staking.",
    );

    msg!("✅ Correctly failed unauthorized staking deposit.");
