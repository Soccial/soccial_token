// ======================================================================
/// Soccial Token – Integration Tests: Vault Withdrawals (Liquidity)
///
/// These tests validate the logic for securely withdrawing tokens 
/// from the liquidity vault in the Soccial Token contract. 
///
/// Covered scenarios:
...
#[tokio::test]
async fn test_withdraw_liquidity_vault_should_succeed() -> Result<(), TransportError> {
    ...
    try_withdraw_liquidity_vault(&mut context, &admin, &participant, 2_000).await?;
...
#[tokio::test]
async fn test_withdraw_liquidity_balances_should_succeed() -> Result<(), TransportError> {
    let (mut context, admin) = setup_test_env().await;
    ...
    try_withdraw_liquidity_vault(&mut context, &admin, &participant, withdraw_amount).await?;
    ...
    assert_eq!(
        ...
        withdraw_amount,
        "❌ Liquidity vault should decrease by {} tokens, but decreased by {}",
        withdraw_amount,
        liquidity_before - liquidity_after
    );
    ...
    msg!("✅ Liquidity vault withdrawal successful and balances verified.");
    log_all_balances(&mut context, &admin).await?;
...
#[tokio::test]
async fn test_withdraw_liquidity_vault_invalid_authority_should_fail() -> Result<(), TransportError> {
    ...
    try_deposit_liquidity_vault(&mut context, &admin, &admin, 500_000).await?;

    create_user_ata(&mut context, &intruder).await?;

    let result = try_withdraw_liquidity_vault(&mut context, &intruder, &intruder, 1_000).await;
