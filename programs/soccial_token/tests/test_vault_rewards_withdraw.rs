// ======================================================================
/// Soccial Token – Integration Tests: Vault Withdrawals (Rewards)
///
/// These tests validate the logic for securely withdrawing tokens 
/// from the rewards vault in the Soccial Token contract. 
///
/// Covered scenarios:
...
#[tokio::test]
async fn test_withdraw_rewards_vault_should_succeed() -> Result<(), TransportError> {
    ...
    // Fund source vault
    context.mint_tokens_to_vault("rewards", 100_000_000_000).await?;

    try_withdraw_rewards_vault(&mut context, &admin, &participant, 2_000).await?;
...
#[tokio::test]
async fn test_withdraw_rewards_balances_should_succeed() -> Result<(), TransportError> {
    let (mut context, admin) = setup_test_env().await;
    ...
    try_withdraw_rewards_vault(&mut context, &admin, &participant, withdraw_amount).await?;
    ...
    assert_eq!(
        ...
        withdraw_amount,
        "❌ Rewards vault should decrease by {} tokens, but decreased by {}",
        withdraw_amount,
        rewards_before - rewards_after
    );
    ...
    msg!("✅ Rewards vault withdrawal successful and balances verified.");
    log_all_balances(&mut context, &admin).await?;
...
#[tokio::test]
async fn test_withdraw_rewards_vault_invalid_authority_should_fail() -> Result<(), TransportError> {
    ...
    try_deposit_rewards_vault(&mut context, &admin, &admin, 500_000).await?;
    

    create_user_ata(&mut context, &intruder).await?;

    let result = try_withdraw_rewards_vault(&mut context, &intruder, &intruder, 1_000).await;
