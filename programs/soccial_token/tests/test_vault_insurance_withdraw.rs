...
/// from the insurance vault in the Soccial Token contract. 
///
/// Covered scenarios:
...
#[tokio::test]
async fn test_withdraw_insurance_vault_should_succeed() -> Result<(), TransportError> {
    ...
    try_withdraw_insurance_vault(&mut context, &admin, &participant, 2_000).await?;
...
#[tokio::test]
async fn test_withdraw_insurance_balances_should_succeed() -> Result<(), TransportError> {
    let (mut context, admin) = setup_test_env().await;
    ...
    try_withdraw_insurance_vault(&mut context, &admin, &participant, withdraw_amount).await?;
    ...
    assert_eq!(
        ...
        withdraw_amount,
        "❌ Airdrop vault should decrease by {} tokens, but decreased by {}",
        withdraw_amount,
        insurance_before - insurance_after
    );
    ...
    msg!("✅ Airdrop vault withdrawal successful and balances verified.");
    log_all_balances(&mut context, &admin).await?;
...
#[tokio::test]
async fn test_withdraw_insurance_vault_invalid_authority_should_fail() -> Result<(), TransportError> {
    ...
    try_deposit_insurance_vault(&mut context, &admin, &admin, 500_000).await?;

    create_user_ata(&mut context, &intruder).await?;

    let result = try_withdraw_insurance_vault(&mut context, &intruder, &intruder, 1_000).await;
