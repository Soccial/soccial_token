...
/// ======================================================================
/// Soccial Token – Integration Tests: Rewards Vault Deposits
///
/// These integration tests validate the logic for securely depositing
/// tokens into the Rewards Vault. The tests ensure that balances are
...
#[tokio::test]
async fn test_deposit_rewards_vault_should_succeed() -> Result<(), TransportError> {
    ...
    try_deposit_rewards_vault(&mut context, &admin, &participant, 1_000).await?;
...
#[tokio::test]
async fn test_deposit_rewards_balances_should_succeed() -> Result<(), TransportError> {
    let (mut context, admin) = setup_test_env().await;
    ...
    // Execute the deposit
    try_deposit_rewards_vault(&mut context, &admin, &participant, deposit_amount).await?;
    ...
    assert_eq!(
        ...
        deposit_amount,
        "❌ Rewards vault should increase by {} tokens, but increased by {}",
        deposit_amount,
        rewards_after - rewards_before
    );
    ...
    msg!("✅ Rewards vault deposit successful and balances verified.");
    log_all_balances(&mut context, &admin).await?;
...
#[tokio::test]
async fn test_deposit_rewards_vault_without_tokens_should_fail() -> Result<(), TransportError> {
    ...
    let result = try_deposit_rewards_vault(&mut context, &admin, &participant, 1_000).await;

    assert!(
        result.is_err(),
        "🚨 Expected failure when depositing rewards without tokens."
    );
...
#[tokio::test]
async fn test_deposit_rewards_vault_zero_tokens_should_fail() -> Result<(), TransportError> {
    ...
    let result = try_deposit_rewards_vault(&mut context, &admin, &participant, 0).await;
...
#[tokio::test]
async fn test_deposit_rewards_vault_unauthorized_should_fail() -> Result<(), TransportError> {
    ...
    let result = try_deposit_rewards_vault(&mut context, &intruder, &intruder, 1_000).await;
    
    assert_custom_error(
        result,
        ErrorCode::Unauthorized,
        "Expected failure when unauthorized user tried to deposit rewards.",
    );

    msg!("✅ Correctly failed unauthorized rewards deposit.");
