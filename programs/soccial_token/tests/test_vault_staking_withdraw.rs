// ======================================================================
/// Soccial Token – Integration Tests: Vault Withdrawals (Staking)
///
/// These tests validate the logic for securely withdrawing tokens 
/// from the staking vault in the Soccial Token contract. 
///
///
...
#[tokio::test]
async fn test_withdraw_staking_vault_should_fail() -> Result<(), TransportError> {
    ...
    // Fund source vault
    context.mint_tokens_to_vault("staking", 100_000_000_000).await?;

    let result = try_withdraw_staking_vault(&mut context, &admin, &participant, 2_000).await;

    // The vault is blocked for withdrawn, it should fail
    assert_custom_error(
        result, 
        VaultError::UnknownVaultType, 
        "Expected Unauthorized vault access. It should return a Unknown vault error because the vault is blocked for withdraw."
    );
...
#[tokio::test]
async fn test_withdraw_staking_vault_invalid_authority_should_fail() -> Result<(), TransportError> {
    ...
    try_deposit_staking_vault(&mut context, &admin, &admin, 500_000).await?;

    create_user_ata(&mut context, &intruder).await?;

    let result = try_withdraw_staking_vault(&mut context, &intruder, &intruder, 1_000).await;
