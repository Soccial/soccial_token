...
/// ======================================================================
/// Soccial Token – Integration Tests: Treasury Vault Deposits
///
/// These integration tests validate the logic for securely depositing
/// tokens into the Treasury Vault. The tests ensure that balances are
...
#[tokio::test]
async fn test_deposit_treasury_vault_should_succeed() -> Result<(), TransportError> {
    ...
    try_deposit_treasury_vault(&mut context, &admin, &participant, 1_000).await?;
...
#[tokio::test]
async fn test_deposit_treasury_balances_should_succeed() -> Result<(), TransportError> {
    let (mut context, admin) = setup_test_env().await;
    ...
    // Execute the deposit
    try_deposit_treasury_vault(&mut context, &admin, &participant, deposit_amount).await?;
    ...
    assert_eq!(
        ...
        deposit_amount,
        "❌ Treasury vault should increase by {} tokens, but increased by {}",
        deposit_amount,
        treasury_after - treasury_before
    );
    ...
    msg!("✅ Treasury vault deposit successful and balances verified.");
    log_all_balances(&mut context, &admin).await?;
...
#[tokio::test]
async fn test_deposit_treasury_vault_without_tokens_should_fail() -> Result<(), TransportError> {
    ...
    let result = try_deposit_treasury_vault(&mut context, &admin, &participant, 1_000).await;

    assert!(
        result.is_err(),
        "🚨 Expected failure when depositing treasury without tokens."
    );
...
#[tokio::test]
async fn test_deposit_treasury_vault_zero_tokens_should_fail() -> Result<(), TransportError> {
    ...
    let result = try_deposit_treasury_vault(&mut context, &admin, &participant, 0).await;
...
#[tokio::test]
async fn test_deposit_treasury_vault_unauthorized_should_fail() -> Result<(), TransportError> {
    ...
    let result = try_deposit_treasury_vault(&mut context, &intruder, &intruder, 1_000).await;
    
    assert_custom_error(
        result,
        ErrorCode::Unauthorized,
        "Expected failure when unauthorized user tried to deposit treasury.",
    );

    msg!("✅ Correctly failed unauthorized treasury deposit.");
