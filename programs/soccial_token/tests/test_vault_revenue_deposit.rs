...
/// ======================================================================
/// Soccial Token – Integration Tests: Revenue Vault Deposits
///
/// These integration tests validate the logic for securely depositing
/// tokens into the Revenue Vault. The tests ensure that balances are
...
#[tokio::test]
async fn test_deposit_revenue_vault_should_succeed() -> Result<(), TransportError> {
    ...
    try_deposit_revenue_vault(&mut context, &admin, &participant, 1_000).await?;
...
#[tokio::test]
async fn test_deposit_revenue_balances_should_succeed() -> Result<(), TransportError> {
    let (mut context, admin) = setup_test_env().await;
    ...
    // Execute the deposit
    try_deposit_revenue_vault(&mut context, &admin, &participant, deposit_amount).await?;
    ...
    assert_eq!(
        ...
        deposit_amount,
        "❌ Revenue vault should increase by {} tokens, but increased by {}",
        deposit_amount,
        revenue_after - revenue_before
    );
    ...
    msg!("✅ Revenue vault deposit successful and balances verified.");
    log_all_balances(&mut context, &admin).await?;
...
#[tokio::test]
async fn test_deposit_revenue_vault_without_tokens_should_fail() -> Result<(), TransportError> {
    ...
    let result = try_deposit_revenue_vault(&mut context, &admin, &participant, 1_000).await;

    assert!(
        result.is_err(),
        "🚨 Expected failure when depositing revenue without tokens."
    );
...
#[tokio::test]
async fn test_deposit_revenue_vault_zero_tokens_should_fail() -> Result<(), TransportError> {
    ...
    let result = try_deposit_revenue_vault(&mut context, &admin, &participant, 0).await;
...
#[tokio::test]
async fn test_deposit_revenue_vault_unauthorized_should_fail() -> Result<(), TransportError> {
    ...
    let result = try_deposit_revenue_vault(&mut context, &intruder, &intruder, 1_000).await;
    
    assert_custom_error(
        result,
        ErrorCode::Unauthorized,
        "Expected failure when unauthorized user tried to deposit revenue.",
    );

    msg!("✅ Correctly failed unauthorized revenue deposit.");
