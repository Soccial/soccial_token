// ======================================================================
/// Soccial Token – Integration Tests: Vault Withdrawals (Offchain Reserve)
///
/// These tests validate the logic for securely withdrawing tokens 
/// from the offchain_reserve vault in the Soccial Token contract. 
///
///
...
#[tokio::test]
async fn test_withdraw_offchain_reserve_vault_should_fail() -> Result<(), TransportError> {
    ...
    let result = try_withdraw_offchain_reserve_vault(&mut context, &admin, &participant, 2_000).await;
...
#[tokio::test]
async fn test_withdraw_offchain_reserve_vault_invalid_authority_should_fail() -> Result<(), TransportError> {
    ...
    try_deposit_offchain_reserve_vault(&mut context, &admin, &admin, 500_000).await?;

    create_user_ata(&mut context, &intruder).await?;

    let result = try_withdraw_offchain_reserve_vault(&mut context, &intruder, &intruder, 1_000).await;
