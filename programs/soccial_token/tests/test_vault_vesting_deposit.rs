...
/// ======================================================================
/// Soccial Token – Integration Tests: Vesting Vault Deposits
///
/// These integration tests validate the logic for securely depositing
/// tokens into the Vesting Vault. The tests ensure that balances are
...
#[tokio::test]
async fn test_deposit_vesting_vault_should_succeed() -> Result<(), TransportError> {
    ...
    try_deposit_vesting_vault(&mut context, &admin, &participant, 1_000).await?;
...
#[tokio::test]
async fn test_deposit_vesting_balances_should_succeed() -> Result<(), TransportError> {
    let (mut context, admin) = setup_test_env().await;
    ...
    let vesting_before = context.get_vault_balance("vesting").await;
    let participant_before = context.get_user_balance(&participant.pubkey()).await;

    // Execute the deposit
    try_deposit_vesting_vault(&mut context, &admin, &participant, deposit_amount).await?;

    context.refresh().await;

    // Balances after the deposit
    let vesting_after = context.get_vault_balance("vesting").await;
    ...
    assert_eq!(
        vesting_after - vesting_before,
        deposit_amount,
        "❌ Vesting vault should increase by {} tokens, but increased by {}",
        deposit_amount,
        vesting_after - vesting_before
    );
    ...
    msg!("✅ Vesting vault deposit successful and balances verified.");
    log_all_balances(&mut context, &admin).await?;
...
#[tokio::test]
async fn test_deposit_vesting_vault_without_tokens_should_fail() -> Result<(), TransportError> {
    ...
    let result = try_deposit_vesting_vault(&mut context, &admin, &participant, 1_000).await;

    assert!(
        result.is_err(),
        "🚨 Expected failure when depositing vesting without tokens."
    );
...
#[tokio::test]
async fn test_deposit_vesting_vault_zero_tokens_should_fail() -> Result<(), TransportError> {
    ...
    let result = try_deposit_vesting_vault(&mut context, &admin, &participant, 0).await;
...
#[tokio::test]
async fn test_deposit_vesting_vault_unauthorized_should_fail() -> Result<(), TransportError> {
    ...
    let result = try_deposit_vesting_vault(&mut context, &intruder, &intruder, 1_000).await;
    
    assert_custom_error(
        result,
        ErrorCode::Unauthorized,
        "Expected failure when unauthorized user tried to deposit vesting.",
    );

    msg!("✅ Correctly failed unauthorized vesting deposit.");
