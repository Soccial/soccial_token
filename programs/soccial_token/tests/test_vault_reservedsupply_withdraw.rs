// ======================================================================
/// Soccial Token – Integration Tests: Vault Withdrawals (Reserved Supply)
///
/// These tests validate the logic for securely withdrawing tokens 
/// from the reserved_supply vault in the Soccial Token contract. 
///
///
...
#[tokio::test]
async fn test_withdraw_reserved_supply_vault_should_succeed() -> Result<(), TransportError> {
    ...
    let result = try_withdraw_reserved_supply_vault(&mut context, &admin, &participant, 2_000).await;
...
#[tokio::test]
async fn test_withdraw_reserved_supply_vault_invalid_authority_should_fail() -> Result<(), TransportError> {
    ...
    try_deposit_reserved_supply_vault(&mut context, &admin, &admin, 500_000).await?;

    create_user_ata(&mut context, &intruder).await?;

    let result = try_withdraw_reserved_supply_vault(&mut context, &intruder, &intruder, 1_000).await;
