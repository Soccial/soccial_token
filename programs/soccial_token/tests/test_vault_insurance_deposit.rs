...
/// ======================================================================
/// Soccial Token – Integration Tests: Insurance Vault Deposits
///
/// These integration tests validate the logic for securely depositing
/// tokens into the Insurance Vault. The tests ensure that balances are
...
#[tokio::test]
async fn test_deposit_insurance_vault_should_succeed() -> Result<(), TransportError> {
    ...
    try_deposit_insurance_vault(&mut context, &admin, &participant, 1_000).await?;
...
#[tokio::test]
async fn test_deposit_insurance_balances_should_succeed() -> Result<(), TransportError> {
    let (mut context, admin) = setup_test_env().await;
    ...
    let insurance_before = context.get_vault_balance("insurance").await;
    let participant_before = context.get_user_balance(&participant.pubkey()).await;

    // Execute the deposit
    try_deposit_insurance_vault(&mut context, &admin, &participant, deposit_amount).await?;

    context.refresh().await;

    // Balances after the deposit
    let insurance_after = context.get_vault_balance("insurance").await;
    ...
    assert_eq!(
        ...
        deposit_amount,
        "❌ Insurance vault should increase by {} tokens, but increased by {}",
        deposit_amount,
        insurance_after - insurance_before
    );
    ...
    msg!("✅ Insurance vault deposit successful and balances verified.");
    log_all_balances(&mut context, &admin).await?;
...
#[tokio::test]
async fn test_deposit_insurance_vault_without_tokens_should_fail() -> Result<(), TransportError> {
    ...
    let result = try_deposit_insurance_vault(&mut context, &admin, &participant, 1_000).await;

    assert!(
        result.is_err(),
        "🚨 Expected failure when depositing insurance without tokens."
    );
...
#[tokio::test]
async fn test_deposit_insurance_vault_zero_tokens_should_fail() -> Result<(), TransportError> {
    ...
    let result = try_deposit_insurance_vault(&mut context, &admin, &participant, 0).await;
...
#[tokio::test]
async fn test_deposit_insurance_vault_unauthorized_should_fail() -> Result<(), TransportError> {
    ...
    let result = try_deposit_insurance_vault(&mut context, &intruder, &intruder, 1_000).await;
    
    assert_custom_error(
        result,
        ErrorCode::Unauthorized,
        "Expected failure when unauthorized user tried to deposit insurance.",
    );

    msg!("✅ Correctly failed unauthorized insurance deposit.");
