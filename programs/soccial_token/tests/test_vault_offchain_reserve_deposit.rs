...
/// ======================================================================
/// Soccial Token – Integration Tests: Offchain Reserve Vault Deposits
///
/// These integration tests validate the logic for securely depositing
/// tokens into the Offchain Reserve Vault. The tests ensure that balances are
...
#[tokio::test]
async fn test_deposit_offchain_reserve_vault_should_succeed() -> Result<(), TransportError> {
    ...
    try_deposit_offchain_reserve_vault(&mut context, &admin, &participant, 1_000).await?;
...
#[tokio::test]
async fn test_deposit_offchain_reserve_balances_should_succeed() -> Result<(), TransportError> {
    let (mut context, admin) = setup_test_env().await;
    ...
    let offchain_reserve_before = context.get_vault_balance("offchain_reserve").await;
    let participant_before = context.get_user_balance(&participant.pubkey()).await;

    // Execute the deposit
    try_deposit_offchain_reserve_vault(&mut context, &admin, &participant, deposit_amount).await?;

    context.refresh().await;

    // Balances after the deposit
    let offchain_reserve_after = context.get_vault_balance("offchain_reserve").await;
    ...
    assert_eq!(
        offchain_reserve_after - offchain_reserve_before,
        deposit_amount,
        "❌ Offchain Reserve vault should increase by {} tokens, but increased by {}",
        deposit_amount,
        offchain_reserve_after - offchain_reserve_before
    );
    ...
    msg!("✅ Offchain Reserve vault deposit successful and balances verified.");
    log_all_balances(&mut context, &admin).await?;
...
#[tokio::test]
async fn test_deposit_offchain_reserve_vault_without_tokens_should_fail() -> Result<(), TransportError> {
    ...
    let result = try_deposit_offchain_reserve_vault(&mut context, &admin, &participant, 1_000).await;

    assert!(
        result.is_err(),
        "🚨 Expected failure when depositing offchain_reserve without tokens."
    );
...
#[tokio::test]
async fn test_deposit_offchain_reserve_vault_zero_tokens_should_fail() -> Result<(), TransportError> {
    ...
    let result = try_deposit_offchain_reserve_vault(&mut context, &admin, &participant, 0).await;
...
#[tokio::test]
async fn test_deposit_offchain_reserve_vault_unauthorized_should_fail() -> Result<(), TransportError> {
    ...
    let result = try_deposit_offchain_reserve_vault(&mut context, &intruder, &intruder, 1_000).await;
    
    assert_custom_error(
        result,
        ErrorCode::Unauthorized,
        "Expected failure when unauthorized user tried to deposit offchain_reserve.",
    );

    msg!("✅ Correctly failed unauthorized offchain_reserve deposit.");
